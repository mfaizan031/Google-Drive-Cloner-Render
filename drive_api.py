...
CLIENT_SECRETS = {
    "web": {
        ...
        "redirect_uris": [
            os.environ.get("REDIRECT_URI", "http://localhost:5000/api/auth/callback") # Default for local dev
        ]
    }
}
...
def login():
    ...
    try:
        ...
        if not CLIENT_SECRETS["web"].get("client_id") or not CLIENT_SECRETS["web"].get("client_secret"):
            return jsonify({
                "error": "OAuth credentials (GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET) are not configured. Please set them as environment variables."
            }), 500
